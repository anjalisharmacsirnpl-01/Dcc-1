...
def check_emp_credentials(email: str, password: str):
    session = SessionLocal()
    try:
        user = session.query(emp_reg_table).filter(
        ...
        if user is not None:
            ...
            user_mapping = dict(user)
            print("User mapping:", user_mapping)
            if user_mapping.get("password") == password:
                print("Password match confirmed.")
                return {
                    "emp_reg_id": user_mapping.get("emp_reg_id"),
                    "email": user_mapping.get("email"),
                    "designation": user_mapping.get("designation")
                }
            else:
                print("Password does not match in mapping.")
        else:
            print("No matching user found for:", email)
        return None
    except Exception as e:
        print("Exception in check_emp_credentials:", e)
        return None
    finally:
        session.close()
